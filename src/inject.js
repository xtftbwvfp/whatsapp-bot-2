WAPI.waitNewMessages(false, async (data) => {
  for (let i = 0; i < data.length; i++) {
    ...
    window.newMessage({user: message.from.user, server: message.from.server, message: message.body, date: Date.now(message.timestamp), type: 'inbound'})
    ...
    if (message.isGroupMsg == true && intents.appconfig.isGroupReply == false) {
      ...
      if (PartialMatch == 0) {
        ...
        window.newMessage({user: message.from.user, server: message.from.server, message: response, date: Date.now(), type: 'reply'})
        return;
      } else {
        return;
      }
            ...
            response.cameras.forEach(element => {
              ...
              toDataURL(url + "?api_id=" + credentials.api_id + "&api_key=" + credentials.api_key)
                .then(dataUrl => {
                  ...
                  window.newMessage({user: message.from.user, server: message.from.server, message: dataUrl, date: Date.now(), type: 'reply'})
                  ...
                  window.newMessage({user: message.from.user, server: message.from.server, message: element.name + ": " + error.message, date: Date.now(), type: 'reply'})
          ...
          if (isNaN(parseInt(message.body))) {
            ...
            .then((resp) => resp.json()).then(function (response) {
              ...
              .then(dataUrl => {
                ...
                window.newMessage({user: message.from.user, server: message.from.server, message: camera.name + ": " + error.message, date: Date.now(), type: 'reply'})
              ...
              response = "Camera not found. Please reply with the number asociated to the camera. If you want to see the camera list, reply with the letter *B*"
              WAPI.sendSeen(message.from._serialized);
              WAPI.sendMessage2(message.from._serialized, response);
              window.newMessage({user: message.from.user, server: message.from.server, message: response, date: Date.now(), type: 'reply'})
            });
